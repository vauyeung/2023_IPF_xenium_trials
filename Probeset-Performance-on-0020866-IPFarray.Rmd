---
title: "Probset performance measurements on slide 0020866"
output: 
  github_document:
    toc: true
  html_notebook:
...
```{r}
ipfArray1.xen <- NormalizeData(ipfArray1.xen)
...
```{r}
ipfArray1.xen <- subset(ipfArray1.xen, subset=nCount_Xenium>20 & nFeature_Xenium>4)
```
...
probes <- intersect(rownames(ipfArray1.xen), rownames(ipfatlas.cells))
```
```{r}
...
nonzeroaverages <- data.frame(gene=probes,
                              xeniumAvg=unlist(future_lapply(probes, function(symbol){
                                counts <- ipfArray1.xen[['Xenium']]$counts[symbol,]
...
nonzeroaverages$xeniumNormAvg <- unlist(future_lapply(probes, function(symbol){
  counts <- GetAssayData(object = ipfArray1.xen, slot = 'data')[symbol,]
  nonzeroavg <- mean(counts[counts>0])
  return(nonzeroavg)
}))
